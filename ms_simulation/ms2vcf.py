...
if __name__ == "__main__":
    if len(sys.argv) >= 3:
        ...
        infile = sys.argv[1]
        outdir = input("please input the outpath")
    elif len(sys.argv) == 1:
        infile = input("please input the infile(ms out)")
        outdir = input("please input the outpath")
...
for num_rep, pos_locus_to_sample_to_genotype_dict in replicate_to_pos_locus_to_sample_to_genotype_dict.items():
	ofn = outdir + '/'+ infile + '_rep{}.vcf'.format(num_rep + 1)
	#ofn = outdir + '/' + 'ms_rep{}.vcf'.format(num_rep + 1)
	with open(ofn, 'w') as fw:
		...
		for pos, sample_to_genotype_dict in pos_locus_to_sample_to_genotype_dict.items():
			vcf_row_list = [infile + 'Unkwn_rep' + str(num_rep + 1), pos, '.', '.', '.', '.', '.', '.', 'GT']
